...
excerpt: "En este articulo veremos como realizar un ataque con una tecnica llamada ret2libc para hacer bypass del NX"
date: 2022-07-29
classes: wide
header:
...
## Contenido
* ¿Que es ret2libc?.
* ¿Que es No Executable (NX)?.
* Un poco de Return Oriented Programming (ROP).
* Analizando nuestro binario.
...
|--------------|
```
Ahora sobreescribimos el ```RIP``` por la direccion ```ret```, entonces ```RSP``` se mueve a la direccion de ```pop rdi; ret```, lo que pasaria seria algo com esto ```ret rip rsp``` y nuestro layout queda asi
...
```
Gef tiene un comando que se llama ```pattern create``` para generar una serie de caracteres que nos ayudaran a calcular los offsets, asi que le pasaremos ese comando de esta manera ```pattern create 100``` y nos generara 100 caracteres, ahora ejecutaremos el binario pasandole esos 100 caracteres como input, si mostramos el stack frame con ```i f``` vemos como nuestro ```saved rip``` tiene esto: ```saved rip = 0x616161616161616a``` ,estos son una serie de caracteres que fueron tomados de la cadena que generamos.

Ahora usaremos otro comando que tiene gef que se llama ```patter offset``` para calcular el offset del RIP, ingresamos lo siguiente

```
patter offset 0x616161616161616a
```
...
```
Para esta practica usaremos el offset 72 que corresponde a little-endian
...
![](/assets/images/ret2libc/libc.png)
...
![](/assets/images/ret2libc/ret.png)
...
![](/assets/images/ret2libc/poprdi.png)
...
![](/assets/images/ret2libc/pwn.png)

Eso ha sido todo, gracias por leer ❤

![](/assets/images/ret2libc/waifu.png)
